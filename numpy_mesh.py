...
def wall(name, pos=[0,0,0],rot=0, dims=[1,1,0.1],holes=[], bandmats=[],bands=[]):
    ...
    for hole in holes:
        ...
        holem[zi,xi]=1
    me = mesh_for_board_hbands(name,xs-l0,ys,zs,holem)
    # create custom properties for storing geometrical data
    me['xs'] = xs.tolist()
    me['zs'] = zs.tolist()
    me['holem'] = holem.tolist()
    ob = bpy.data.objects.new(me.name,me)
    ob.location = pos
...
#dims = [6,0.1,3]
#bands = [[[0,0.2,0.21],[0.1,0.1,0]],[[2.4,3],[0,0]]]
#bandmats = [mat1,mat2,mat3]
#name = 'miparedzotapintada'
